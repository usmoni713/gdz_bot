        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self.url) as response:
                ...
                if status == 200:
                    ...
                    logger.warning(f"Can't to get response with num page | num: {self.url} | response status: {status}")
                    return {}
...
class Book:
    def __init__(
            self, title: str = '', url: str = '', cover_url: str = '', type_: str = '',
            authors: list[str] | tuple[str] = ('', ''), publisher: str = '') -> None:
        self.title = title
        self.url = url
    ...
    async def get_num_structure(self) -> dict:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(self.url) as response:
                ...
                if status == 200:
                    ...
                    logger.warning(f"Can't to get response with book page | book: {self.url} | response status: {status}")
                    return {}
        soup: bs4.BeautifulSoup = bs4.BeautifulSoup(html, "lxml")
        ...
        def __rec(target) -> dict:
            ...
            for div_section in target:
                ...
                while r_section_name == "*stub":
                    h_index += 1
                    div_section_name = div_section.findChildren(f"h{h_index}", {"class": "heading"})
                    if div_section_name:
                ...
                if div_section.find("div", recursive=False):
                    ...
                    for div_num in div_section.find("div", recursive=False).find_all('a'):
                        r_num_list.append(Number(div_num.get("title"), f"https://gdz.ru{div_num.get("href")}"))
                    r_dict[r_section_name] = r_num_list
                elif div_section.find("section", recursive=False):
                    r_dict[r_section_name] = {}
                    r_dict[r_section_name] = __rec(div_section.find_all("section", recursive=False))
                else:
                    return {}
            # return result
            return r_dict
        ...
        # Parsing
        result = __rec(soup.find("div", {"class": "task__list"}).find_all(
            "section",
            {"class": "section-task"},
            recursive=False
        ))
...
class Chapter:
    def __init__(
            self,
            class_: int = 0, subject: str = '', url: str = '',
    ...
    async def get_books(self) -> list[Book]:
        ...
        for div_book in div_book_list.find_all('a', {"class": "book__link"}):
            ...
            for div_book_attr in div_book.find("div", {"class": "book__description"}).findChildren(recursive=False):
                if div_book_attr.get("data-book-type"):
                    ...
                    book_authors = div_book_attr.find("span", {"itemprop": "author"}).text.split(", ")
                elif div_book_attr.find("span", {"itemprop": "publisher"}):
                    book_publisher = div_book_attr.find("span", {"itemprop": "publisher"}).text
...
class Api:
    ...
    async def get_chapter(chapter_class: int = 0, chapter_subject: str = '') -> Chapter:
        ...
        """
        # The main things
        if 1 > chapter_class > 11 or type(chapter_class) is not int or \
                chapter_subject == '' or type(chapter_subject) is not str:
            return Chapter()
        async with aiohttp.ClientSession() as session:
            async with session.get("https://gdz.ru./") as response:
                ...
                if status == 200:
                    ...
                    logger.warning(f"Can't to get response with main gdz page | response status: {status}")
                    return Chapter()
        soup: bs4.BeautifulSoup = bs4.BeautifulSoup(html, "lxml")
        # Parsing
        div_classes: bs4.element.ResultSet[bs4.element.Tag] = soup.find(
            "div",
            {"class": "sidebar__main"}
        ).find("div").findChildren(recursive=False)
        div_subjects: bs4.element.ResultSet[bs4.element.Tag] = div_classes[chapter_class - 1].find_all(
            'a',
            {"class": "link link-sub"}
        )
        ...
        logger.warning(f"Chapter not found | input: class - \"{chapter_class}\", subject - \"{chapter_subject}\"")
        return Chapter()
